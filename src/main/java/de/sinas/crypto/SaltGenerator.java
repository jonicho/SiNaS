...
public class SaltGenerator {
    ...
    public static byte[] generateSalt(String username, String password, HashHandler hashHandler) {
        ...
        for (int i = 0; i < 64; i++) {
            ...
            byte[] thisColumn;
            if (i > 0) {
                thisColumn = hashHandler.getCheckSum((new String(uHash) + new String(pHash)).getBytes());
            } else
        ...
        return hashHandler.getSecureHash(new String(comboHash), snp);
    }
    ...
    private static void mixColumns(byte[][] arr, int[][] galois) {
    ...
    private static int mcHelper(byte[][] arr, int[][] g, int i, int j) {
        int mcsum = 0;
        for (int k = 0; k < 4; k++) {
            int a = g[i][k];
            int b = arr[k][j];
    ...
    private static int mcCalc(int a, int b) {
        int[][] mc2 = {
    ...
    private static byte[] leftrotate(byte[] arr, int times) {
        assert (arr.length == 4);
        if (times % 4 == 0) {
            return arr;
        }
